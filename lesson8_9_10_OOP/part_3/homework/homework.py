...
a) содержит атрибут "name" строкового типа
b) в __init__ определяется 1 атрибут с тем же именем - "name"
c) имеет 3 родителя (множественное наследование), у каждого из родительских классов тоже есть атрибут с именем "name"
d) *переопределяется магический метод __getattribute__ cледующим образом:
-- если атрибут найден в инстансе класса - печатаем f"Attribute {name} found in Instance"
-- если атрибут НЕ найден в инстансе класса, но найден в самом классе - печатаем f"Attribute {name} found in Instance Class " + имя класса
-- если атрибут НЕ найден в инстансе класса и в самом классе, но найден в каком-либо из родительских классов
   - печатаем f"Attribute {name} found in Instance Parent Class " + имя родительского класса
-- атрибут не найден вообще - порождаем исключение AttributeError с соответствующим текстом ошибки

  (ПОДСКАЗКА: для поиска атрибутов можно использовать __dict__ или же встроенные функции hasattr и getattr)
  
e) содержит магические методы __getitem__ и __setitem__, чтобы у инстанса можно было получить/установить значение атрибута через instance["name"]
   (ПРИМЕЧАНИЕ: __getitem__ должен учитывать уже реализованную в __getattribute__ логику получения атрибута)
   
"""

# ПРИМЕЧАНИЕ: нужно будет проверить работу ваших магических методов, через динамическое удаление атрибутов
